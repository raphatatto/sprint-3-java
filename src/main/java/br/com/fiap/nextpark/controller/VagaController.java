...
public class VagaController {
    ...
    public String nova(Model model) {
        ...
        model.addAttribute("status", StatusVaga.values());
        return "vaga/form";
    }

    @PostMapping
    public String criar(@ModelAttribute Vaga v, RedirectAttributes ra) {
        vagaService.salvar(v);
        ra.addFlashAttribute("msg","Vaga criada!");
        return "redirect:/vaga";
    }

    @GetMapping("/{id}/editar")
    public String editar(@PathVariable Long id, Model model) {
        model.addAttribute("vaga", vagaService.obter(id));
        model.addAttribute("status", StatusVaga.values());
        return "vaga/form";
    }
    ...
    public String atualizar(@PathVariable Long id, @ModelAttribute Vaga v, RedirectAttributes ra) {
        vagaService.atualizar(id, v);
        ra.addFlashAttribute("msg","Vaga atualizada!");
        return "redirect:/vaga";
    }

    @PostMapping("/{id}/delete")
    public String deletar(@PathVariable Long id, RedirectAttributes ra) {
        vagaService.deletar(id); // motos nessa vaga viram DESALOCADAS
        ra.addFlashAttribute("msg","Vaga excluída!");
        return "redirect:/vaga";
    }
}
